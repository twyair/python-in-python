...
@po.pyclass("function")
@dataclass
class PyFunction(po.PyClassImpl, po.PyValueMixin):
    code: PyRef[PyCode]
    globals: PyDictRef
    ...
    def new(
        ...
        name = code.payload.code.obj_name
        return PyFunction(
            code=code,
            globals=globals,
            closure=closure,
    ...
    def into_object(self: PyFunction, vm: VirtualMachine) -> PyRef[PyFunction]:
        return PyRef.new_ref(self, vm.ctx.types.function_type, None)
...
@po.pyclass("method")
@dataclass
class PyBoundMethod(po.PyClassImpl, po.PyValueMixin):
    object: PyObjectRef
    ...
    # TODO: impl Callable for PyBoundMethod
